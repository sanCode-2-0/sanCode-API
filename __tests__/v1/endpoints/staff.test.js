...
describe("Test GET /staff/:idNo", () => {
  ...
  test("Properties of response body", async () => {
    const response = await request(app).get(`/staff/${testIDNumber}`);
...
describe("Test GET /staff-data", () => {
  ...
  test("Status 200 - Successful", async () => {
    await request(app).get("/staff-data").expect(200);
  });
  ...
  test("Properties in the received JSON", async () => {
    const response = await request(app).get("/staff-data").expect(200);
...
describe("Test POST /staff-create-entry", () => {
  const testPostData = {
    idNo: 21772421,
    fName: "Mildred",
    sName: "Mulima",
